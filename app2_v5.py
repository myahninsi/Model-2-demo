...
if not os.path.exists(company_data_path):
    ...
    if st.button('Predict'):
        ...
        def load_data(ticker):
            try:
                data = yf.download(ticker, period='1y', progress=False)  # Use 1 year of data for quicker results
        ...
        if data.empty:
            ...
            fig, ax = plt.subplots()
            sns.heatmap(confusion_mtx, annot=True, fmt="d", cmap="Blues", xticklabels=['Down', 'Up', 'Neutral'], yticklabels=['Down', 'Up', 'Neutral'], ax=ax)
            st.pyplot(fig)
